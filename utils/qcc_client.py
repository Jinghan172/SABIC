...
_MFG_WORDS   = ["制造","生产","研发","加工","生产加工","制备","合成","提炼","铸造","配制"]
_TRADE_WORDS = ["销售","经销","代理","批发","零售","贸易","进出口","供应"]

def classify_role(scope: str) -> str:
    """从经营范围文本判断企业角色：manufacturer / trader / both / unknown"""
    if not scope:
        return "unknown"
    has_mfg   = any(w in scope for w in _MFG_WORDS)
    has_trade = any(w in scope for w in _TRADE_WORDS)
    if has_mfg and has_trade: return "both"
    if has_mfg:               return "manufacturer"
    if has_trade:             return "trader"
    return "unknown"
